...
function AuthWrapper() {
  // token shape: { accessToken, refreshToken, username, expiresAt }
  const [tokens, setTokens] = useLocalStorage("hytech_tokens", null);
  const [loading, setLoading] = useState(true);
  const [offlineDemo, setOfflineDemo] = useState(false);

  // Helper: store tokens in localStorage via hook
  const saveTokens = (t) => setTokens(t);

  const clearTokens = () => setTokens(null);

  // Attempt to refresh tokens on mount if we have a refresh token
  useEffect(() => {
    let mounted = true;
    async function tryRefresh() {
      // If the URL contains verified=1 (came from email verification), attempt a refresh using the cookie
      ...
      if (justVerified) {
        try {
          ...
          if (resp.ok) {
            ...
            const newTokens = { accessToken: data.accessToken, refreshToken: data.refreshToken || tokens?.refreshToken, username: username, expiresAt: Date.now() + (data.expiresIn || 3600) * 1000 };
            saveTokens(newTokens);
            // remove query param to avoid repeated attempts
      ...
      try {
        ...
        // expected { accessToken, refreshToken, expiresIn }
        let username = tokens.username || '';
        if (!username && data.accessToken) {
          try {
            const parts = data.accessToken.split('.');
        ...
        if (mounted) saveTokens(newTokens);
      } catch (err) {
        // backend might be down — fall back to offline demo mode
        console.warn("Auth refresh failed, entering offline demo mode", err);
        setOfflineDemo(true);
        clearTokens();
      } finally {
  ...
  // If the URL has a verification token, show the verify screen
  useEffect(() => {}, []);
  const params = new URLSearchParams(window.location.search);
  const verifyToken = params.get('token') || params.get('verify_token');
  ...
  body: JSON.stringify(payload)
      ...
      // network error — offline demo fallback
      console.warn("Login request failed, falling back to offline demo", err);
      setOfflineDemo(true);
      return { ok: true, offline: true };
    }
  };
  ...
  // fetch helper that auto-refreshes if access token expired
  const fetchWithAuth = async (input, init = {}) => {
    if (offlineDemo) return fetch(input, init);
    let cur = tokens;
      ...
      try {
        ...
        if (r.ok) {
          ...
          cur = {
            ...
            refreshToken: d.refreshToken || cur.refreshToken,
            expiresAt: Date.now() + (d.expiresIn || 3600) * 1000,
          };
          saveTokens(cur);
        } else {
          // refresh failed — clear and fallback
          clearTokens();
  ...
  const headers = new Headers(init.headers || {});
    if (cur?.accessToken) headers.set("Authorization", `Bearer ${cur.accessToken}`);
  ...
  if (offlineDemo) {
    // render original demo flow when offline or API unavailable
    return <Login onLogin={({ username }) => { saveTokens({ username, offline: true }); return true; }} />;
  }
  ...
  // Pass fetchWithAuth and logout to the App so it can call protected APIs
  return <App auth={{ username: tokens?.username, loggedIn: !!tokens }} onLogout={logout} fetchWithAuth={fetchWithAuth} />;
}
